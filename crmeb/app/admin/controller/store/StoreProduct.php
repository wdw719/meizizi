...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            //添加商品品牌
            Form::select('brand','品牌')->setOptions(function(){
                $brand = new GoodsBrand();
                $brand_list = $brand -> brandAllList();
                $menuss=[];
                foreach ($brand_list as $v){
                    $menuss[] = ['value'=>$v['id'],'label'=>$v['brand_name']];
                }
                return $menuss;
            })->filterable(1)->multiple(1),
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            //商品品牌
            Form::select('brand','品牌',explode(',',$product->getData('brand')))->setOptions(function(){
                $brand = new GoodsBrand();
                $list = $brand -> brandAllList();
                $menus=[];
                foreach ($list as $menu){
                    $menus[] = ['value'=>$menu['id'],'label'=>$menu['brand_name']];
                }
                return $menus;
            })->filterable(1)->multiple(1),
